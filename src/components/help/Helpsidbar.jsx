...
export default function HelpSidebar() {
  return (
    <aside className="bg-[#222325] lg:min-h-screen pt-6 lg:py-6 px-4">
      <h1 className="font-medium text-[#ACADAE] text-[16px] mb-4 sidebar-title hidden custom:block">
        Settings
      </h1>

      <nav className="flex lg:flex-col justify-between gap-3">
        {navItems.map((item) => (
          <div
            key={item.name}
          ...
          >
            ...
            <span className="absolute left-full top-11 border border-[#35373a7e] bg-[#1B1D1F] text-white text-xs py-1 px-2 ml-[-65px] rounded opacity-0 group-hover:opacity-100 transition-opacity lg:hidden whitespace-nowrap z-50">
              {item.name}
            </span>
          </div>
        ))}
