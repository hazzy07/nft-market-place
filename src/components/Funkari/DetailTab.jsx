...
const AccordionItem = ({ title, children, initialOpen = false }) => {
  ...
  return (
    <div className="bg-black py-2 border border-[#48484ba9] mt-2 rounded-md ">
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center  justify-between w-full px-6 text-white font-inter font-[400] text-[14px]"
      >
        {title}
        <RxCaretDown
          size={21}
      ...
      {open && <div className="px-6 pt-3">{children}</div>}
    </div>
  );
};
