...
import { ChrisBear, GreenBear, LilPudgys, PurpleBear } from "../../assets/Home/HomeRoute";
...
export default function HomeCarousel() {
  ...
  return (
    <div className="relative w-full  overflow-hidden rounded-lg ">
      ...
      >
        {slides.map((slide, idx) => (
          <div key={idx} className="w-full flex-shrink-0 relative">
            ...
            <div className="absolute w-[100%] bottom-0 flex flex-col md:flex-row justify-between items-end p-4 md:p-6 bg-gradient-to-t from-black/70 to-transparent rounded-lg">
                ...
                <div className="bg-[#ffffff23] border-[#FFFFFF29] border flex justify-between flex-wrap gap-3 md:p-3 p-2 rounded-lg text-white text-xs md:text-sm">
                  {slide.floorPrices.map((price, index) => (
                    <div key={index} className="space-y-1">
                      <span className="font-[400] font-[space] md:text-[14px] text-[12px] text-[#FFFFFF] ">{price.name}</span>
                      <div className="flex justify-between font-[400] font-[space] md:text-[14px] text-[12px] text-[#FFFFFF]">
                        <p>{price.value}</p>
      ...
      <button onClick={prevSlide} className="absolute top-1/2 left-3 transform -translate-y-1/2 text-white p-2 rounded-full hover:bg-black/50">
        <RxCaretLeft size={24} />
      </button>
      <button onClick={nextSlide} className="absolute top-1/2 right-3 transform -translate-y-1/2 text-white p-2 rounded-full hover:bg-black/50">
